...
% RESOLUÇÃO DA PARTE 1 DA PROVA 2 DE 2023.1 DE CÁLCULO NUMÉRICO
% ALUNA: MAÍSA GARCIA NEPOMUCENO CORRÊA

syms x;
F1 = x^5 - 3.56*x^4 + 11.2105*x^3 + 5.83825*x^2 - 0.3845*x + 0.00325;
symX = sym('x');

% PRIMEIRA QUESTAO
fprintf("Primeira Questão:\n\n");
coef1 = [1 -3.56 11.2105 5.83825 -0.3845 0.00325];
n1 = length(coef1)-1;
...
% 0.00091209 4.56 -2.8006 -0.062076
...
plotF(0, 0.1, 500, symX, F1); 
plotF(-1, 0, 500, symX, F1); 

fprintf("Analisando o gráfico do limite das raízes positivas, temos que ξ1 ∈ [0.0,0.2]\n");
fprintf("e ξ2 ∈ [0.4,0.6]");
fprintf("\nAnalisando o gráfico do limite das raízes negativas, temos que ξ3 ∈ [-0.6,-0.4]");

fprintf("\n\nLetra D:\n");
% LETRA D: Todas as raízes reais utilizando todos os métodos com precisão de 10e-5
toler = 10e-5;
maxIter = 50;
aproxRaiz = [0.01, 0.05, -0.45];
w = length(aproxRaiz);
...
fprintf("\nPara a raiz aproximada 0.01:\n");
disp(C1);
fprintf("Ou seja, temos que a raiz ou é muito próxima, ou é 0.01.\n"); 
...
fprintf("\nPara a raiz aproximada 0.05:\n");
disp(C2);
fprintf("Ou seja, temos que a raiz ou é muito próxima, ou é 0.05.\n"); 
...
fprintf("\nPara a raiz aproximada -0.45:\n");
disp(C3); 
fprintf("Ou seja, temos que a raiz ou é muito próxima, ou é -0.5.\n"); 

fprintf("\n\nLetra E:\n");
% LETRA E: Em caso de raízes complexas, utilize o método de Muller, secante ou Newton para 
% determinar pelo menos um par conjugado delas. Utilize o intervalo [2 − 4𝑖, 2 + 4𝑖]
ai = 2 - 4i;
bi = 2 + 4i;
...
disp(raizSI);
fprintf("Analisando os resultados, temos que a raiz imaginária ou é muito próxima de\n");
fprintf("2 + 3i ou é 2 + 3i; logo tem como par conjugado 2 - 3i");

% SEGUNDA QUESTAO
fprintf("\n\nSegunda Questão:\n\n");
syms j;
F2 = 50*j+(25.85e-3*log(j/9.4e-12))-5; 
symJ = sym('j');

plotF(0, 0.1, 500, symJ, F2);
[raiz, iter, erro] = metNewton(0.89, toler, maxIter, symJ, F2);
fprintf("Usando o método de Newton e chutando a raiz em 0.89 (pelo gráfico) temos que I = %.7f", raiz);
