...
% RESOLUÇÃO DA PARTE 2 DA PROVA 2 DE 2022.2 DE CÁLCULO NUMÉRICO
...
plotF(0.5, 2, 500, symX, F1); 
plotF(-5, -0.61, 500, symX, F1);
% 1 -3

fprintf("Analisando o gráfico do limite das raízes positivas, temos que ξ1 ∈ [0.5,2]\n");
fprintf("\nAnalisando o gráfico do limite das raízes negativas, temos que ξ2 ∈ [-5,-0.61]");
...
fprintf("\nPara a raiz aproximada 1:\n");
disp(C1);
fprintf("Ou seja, temos que a raiz ou é muito próxima, ou é 1.\n"); 
...
fprintf("\nPara a raiz aproximada -3:\n");
disp(C2);
fprintf("Ou seja, temos que a raiz ou é muito próxima, ou é -3.\n"); 

fprintf("\n\nLetra E:\n");
% LETRA E: Em caso de raízes complexas, utilize o método de Muller, Secante ou Newton para 
% determinar pelo menos um par conjugado delas. Utilize o intervalo [2 - 𝑖, 2 + 𝑖]
ai = 2 + 1i;
bi = 2 - 1i;
x0 = ai + (bi - ai) / 2;
% [raizSI, iterSI, ~] = metSecante(ai, bi, toler, maxIter, symX, F1)
[raizN, iterN, ~] = metNewton(x0, toler, maxIter, symX, F1);   
fprintf("Pelo método de Newton, com valor inicial 2 - i, e usando %d iterações temos que a raiz procurada é: ", iterN);
disp(raizN);

% SEGUNDA QUESTAO
% Encontre uma raiz não nula por um método de sua escolha com precisão de 10e-5, para a equação abaixo
fprintf("\n\nSegunda Questão:\n\n");
syms j;
F2 = j^2 - 4*sin(j);
symJ = sym('j');

plotF(0, 10, 500, symJ, F2);
[raiz, iter, erro] = metNewton(1.8, toler, maxIter, symJ, F2);
fprintf("Usando o método de Newton e chutando a raiz em 1.8 (pelo gráfico) temos que I = %.7f", raiz);
